...
| 22 | `22-coin-store-and-reward-tiering.md` | Re-tier the 8 best card themes out of the buyable Shop into streak-milestone + achievement rewards; give every milestone day a bonus spin wheel; re-tune the freeze economy; introduce a coin-pack store (UI-only, payments stubbed). Builds on `18`/`19`/`20`/`21` (gamification, card themes, spin wheel, achievements). | ✅ **All 3 phases built** (2026-07-20), Babel- & catalogue-verified; on-device pending. **No DB changes anywhere** — all `lib/*.js` data + client UI. P1: new `achievement` tier + 8 themes re-tiered (2→milestone/legendary, 6→trophy grants via `claimTrophy`); first-week ladder reshuffled; **new day-300 milestone** (`MILESTONES`); Daybreak recolored. P2: `FREEZE_COST` 500→**3,000**; new `FREEZE_OVERFLOW_COINS` (overflow freezes → coins via a shared `clampFreezeGrant` helper); spin wheels added to days 100–1000 (`MILESTONE_THEME_GRANTS` emptied, all theme grants now via `claimSpin`). P3: new **`lib/coins.js`** (`COIN_PACKS`); Shop "Freeze" tab → **"General"** (coin packs + freeze); Buy stubbed to an info toast (same as Pro); Buy-coins shortcut on unaffordable themes. **Reverses `IDEAS-gamification.md`'s "purchasable coins rejected"** — see that doc's updated note. |
