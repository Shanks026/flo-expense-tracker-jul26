...
const THEMES_RAW = [
  ...
  // Day 1's wheel (and its theme grant via the wheel — was Nebula, removed
  // 2026-07-20, then Ocean Deep) was REMOVED entirely 2026-07-22
  // (FINDINGS-rank-ladder-rollout.md Finding 1) — day 1 no longer grants a
  // theme via the wheel. Ocean Deep itself moved to the Rank rewards section
  // below (now Saver's reward, same day, same edit) rather than being left
  // behind as a plain purchasable Common theme — see its entry there.
  {
    id: "ruby",
    name: "Ruby",
    tier: "milestone",
  ...
  // === Rank rewards (27-rank-ladder-rework.md Phase 2; Saver added
  // 2026-07-22, FINDINGS-rank-ladder-rollout.md) ===
  // Four now, not three — Meridian/Heartwood/Sovereign one per movement of the
  // ladder (#5/#7/#9) so reaching a themed rank stays an event; Ocean Deep is
  // the exception, deliberately: Saver is reached instantly by construction
  // (everyone starts there), so it isn't an "event" reward the way the other
  // three are. It's the same guaranteed-on-day-one grant Ocean Deep always
  // was (previously via the now-removed day-1 spin wheel — Finding 1), just
  // moved onto the Saver rank-claim instead of a wheel spin. Reusing the
  // existing theme rather than commissioning a new one for a reward everyone
  // gets for free the moment they open the app. Colour families for
  // Meridian/Heartwood/Sovereign were picked against the existing catalogue
  // rather than in isolation: blues, reds/ambers, purples and metals are all
  // crowded, so these take the three genuinely under-used spaces (slate-line,
  // forest→gold, and wine→gold) and none of them reads as a recolour of an
  // existing card.
  {
    id: "ocean-deep",
    name: "Ocean Deep",
    tier: "rank",
    cost: 0,
    // Saver (#1, instant) — moved here from a plain purchasable Common theme
    // (was `tier: "common", cost: 400`) once it became Saver's reward, so it
    // follows the same "never purchasable" rule the other three rank themes
    // already do rather than being the one exception.
    unlock: { type: "rank", rankId: "saver", label: "Saver" },
