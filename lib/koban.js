...
//
// Lowercase source string (not `${streak}-DAY STREAK`) — StreakCelebration
// applies textTransform:'capitalize' itself now (no more pill, plain large
// text, same convention RankUpCelebration's own eyebrow uses), which only
// title-cases each word and leaves already-uppercase letters untouched — so
// the source has to actually BE lowercase for that to do anything.
export function recapEyebrow({ streak, isNewStreak }) {
  if (isNewStreak) return null;
  return `${streak}-day streak`;
}

// Today Card copy (18-gamification-ritual-and-ledger.md Phase 3) — the
// close-the-day ritual's three states. Same voice rules as everywhere else in
// this file: terse, no exclamation marks, never shame the user for spending —
// a logged day's body states the count plainly, it doesn't editorialise about
// the amount.
export function todayCardCopy({ state, todayTotals }) {
  if (state === 'logged') {
    const n = todayTotals?.count ?? 0;
    return { title: 'Day closed', body: `${n} logged today.` };
  }
  if (state === 'nospend') {
    return { title: 'No-spend day', body: 'Nothing to log — nice.' };
  }
  return { title: 'Nothing logged yet', body: 'Add something, or mark today no-spend.' };
}
