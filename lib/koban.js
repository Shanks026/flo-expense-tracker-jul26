...
const RECAP_NOTIFICATION_POOLS = {
  new_streak: [
    (_c, t) => ({ title: '🐱 Day 0 — streak started', body: summaryLine(t) }),
    (_c, t) => ({ title: '🐱 First entry logged', body: `${summaryLine(t)}. Day 0.` }),
  ],
  ongoing: [
  ...
  milestone: [
    ...
    (c, t) => ({ title: `🎉 Day ${c}. Koban is impressed.`, body: summaryLine(t) }),
  ],
};
...
// component — lib/streak.js's own comment says the Day-0 relabeling belongs
// here, not in the model, and this is the surface that has to honour it:
// `current` is 1 on the day a streak starts, but the user is shown "Day 0".
//
...
export function streakHeadline({ current, loggedToday, isNewStreak }) {
  ...
  if (isNewStreak) return "Day 0 — your streak begins";
