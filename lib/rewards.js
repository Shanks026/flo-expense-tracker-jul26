...
// One-time new-user welcome bundle (28-onboarding-welcome-bundle.md) — granted
// once, at onboarding completion (lib/onboarding.js's finish()), and revealed
// on the "You're set" done screen. This is a DELIBERATE, bounded exception to
// two of this file's standing rules — "coins stay scarce" (invariant 4) and
// "ranks never pay" (27-rank-ladder-rework.md) — made by the product owner
// specifically to soften a new user's stingy first-session economy (25 coins
// for a first log, cheapest theme 400) and cut early churn. It is NOT a rank
// reward and NOT a general coin faucet: it fires exactly once per account, at
// signup, keyed `welcome_bundle` (see claimWelcomeBundle in
// lib/rewardsMutations.js). The theme is Glitch (an Epic Shop theme, id
// 'glitch') — the bundle's grant is separate from Saver's own rank theme
// (Ocean Deep, RANK_THEME_GRANTS.saver), which stays its own thing.
export const WELCOME_BUNDLE = { coins: 3000, freezes: 2, themeId: 'glitch' };
...
//
// Days 3/7/10 added Phase 2 — the "front-loaded first week" from
// IDEAS-gamification.md: the highest-churn window of a new user's life, now
// with a real reward (not just streak recognition) at every one of these days.
// Day 1 originally rode StreakCelebration's existing isNewStreak trigger (NOT
// a MILESTONES entry — day 1 was never a streak milestone) with its own wheel
// below; REMOVED 2026-07-22 (FINDINGS-rank-ladder-rollout.md Finding 1, direct
// feedback: "too much for a new user's very first day"). `ocean-deep` (the
// theme day 1 used to grant) moved to RANK_THEME_GRANTS.saver below — same
// guaranteed day-one grant, now via claimRank instead of the wheel — so
// nothing is orphaned. Day 1 now gives a plain streak celebration with no
// wheel, same as any other non-milestone day.
//
...
export const SPIN_WHEELS = {
  // Day 1 entry REMOVED 2026-07-22 — see this const's own comment above
  // (FINDINGS-rank-ladder-rollout.md Finding 1). spinWheelFor(1) now returns
  // undefined, so day 1 is a plain streak celebration with no wheel.
  3: {
...
// Four ranks now, not three — `saver` added 2026-07-22
// (FINDINGS-rank-ladder-rollout.md, direct feedback: "since we dropped the
// spin, we can move the ocean-deep theme to saver as reward"). Saver is
// reached instantly by construction, so unlike the other three this isn't a
// multi-month "event" reward — it's the same guaranteed grant Ocean Deep
// always gave on day 1 via the now-removed spin wheel (Finding 1), just
// granted through claimRank instead. The other three still carry the
// "reward at every movement of the ladder, not every rank" rule.
export const RANK_THEME_GRANTS = {
  saver: 'ocean-deep', // "Saver" — #1, instant
