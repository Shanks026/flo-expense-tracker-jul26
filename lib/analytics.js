...
export function computeBiggestTransaction(transactions) {
  // Income or expense — but never a transfer. A transfer between your own
  // accounts isn't a real transaction in the "biggest" sense. (Every other
  // analytics fn already filters by exact type, so transfers never reach them;
  // this reduce over the raw list is the one that must exclude them explicitly.)
  const real = transactions.filter((tx) => tx.type === 'income' || tx.type === 'expense');
  if (!real.length) return null;
  return real.reduce((max, tx) => (tx.amount > max.amount ? tx : max), real[0]);
}

// Per-DATE daily spending for the chart — a trailing `days`-day window, NOT
// the old weekday aggregation (which summed every Monday together etc., and
// whose single-letter M/T/W/T/F/S/S labels were ambiguous — Tue/Thu and
// Sat/Sun were indistinguishable). Each bar is now one real calendar date.
//
// Window rule (same for month and custom modes): it ends on the most recent
// RELEVANT day — today when the selected period includes it (the current
// month, or a custom range ending today/later), otherwise the period's own
// end (a past month shows its last 7 days). So the current day is always the
// rightmost bar when it's in range. `start` is clamped to `from` so the chart
// never renders days outside the fetched period as misleading zeros — early
// in a month there may be fewer than `days` bars, which is honest.
//
// occurred_at is a 'yyyy-MM-dd' string, so bars are keyed by that exact string
// (same direct-match approach computeTrend uses) — no new Date()/timezone
// round-trip that could shift a transaction onto the wrong day.
export function computeDailySpending(transactions, from, to, { days = 7, now = new Date() } = {}) {
  const anchor = isAfter(to, now) ? now : to;
  const windowStart = subDays(anchor, days - 1);
  const start = isBefore(windowStart, from) ? from : windowStart;

  const buckets = eachDayOfInterval({ start, end: anchor }).map((date) => ({
    date,
    amount: 0,
    isLast: false,
  }));
