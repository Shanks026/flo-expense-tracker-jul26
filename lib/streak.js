...
// How many trailing days the in-app calendar strip shows
// (05-koban-engagement.md Phase 4). Independent of hooks/useStreak.js's
// 90-day fetch WINDOW_DAYS — that's how far back we query, this is how far
// back we display; the fetch window just needs to be >= this.
...
export function computeStreak(rows, now) {
  ...
  // StreakCalendar's strip and StreakCelebration's animated reveal
  ...
  return {
    ...
    //
    // Display convention (user's call, see 05-koban-engagement.md Phase 2):
    // this moment is shown to the user as **"Day 0 — you've started a
    // streak"**, NOT "Day 1". `current` still counts it as 1 internally, so
    // tomorrow reads 2, milestones stay plain integer comparisons, and
    // `longest` stays a normal count. Only the *label* for this origin moment
    // is Day 0 — that relabeling lives in the copy layer (lib/koban.js), not
    // here.
