...
// FREEZE_CAP is no longer imported here — the cap is rank-derived now and
// freezeCapForRank() owns the base value internally (27-rank-ladder-rework.md
// Phase 2). It stays exported from lib/rewards.js for the UI surfaces that
// still display a cap.
import { REWARDS, FREEZE_COST, FREEZE_OVERFLOW_COINS, MILESTONE_REWARDS, MILESTONE_THEME_GRANTS, TROPHY_REWARDS, SPIN_WHEELS, RANK_THEME_GRANTS, WELCOME_BUNDLE, freezeCapForRank, rankFromXp } from './rewards';
...
// now in one place.
// The cap is rank-derived as of 27-rank-ladder-rework.md Phase 2, and `xp` is
// pulled in the SAME round trip this function already made for `freezes` —
// v_reward_balances exposes coins/xp/freezes together, so reading the rank
// costs nothing extra. That is deliberately NOT the shape the feature doc
// first proposed (passing a cap in from each caller): every caller would have
// had to remember, and a single missed one would have silently reverted that
// path to the flat base cap — a wrong number with no error to notice.
async function clampFreezeGrant(requested) {
  ...
  const cap = freezeCapForRank(rankFromXp(balance?.xp ?? 0).current);
...
export async function buyFreeze() {
  const { data: balance, error: balanceError } = await supabase
    .from('v_reward_balances')
    // `xp` joins the existing select for the rank-derived cap — see
    // clampFreezeGrant above for why the cap is read here rather than passed in.
    .select('coins, freezes, xp')
    .maybeSingle();
  ...
  if (freezes >= cap) return { error: new Error('freeze_cap_reached') };
...
export async function claimMilestone(day) {
  ...
  const { data, error } = await supabase
    .from('reward_events')
    .upsert(
      // `xp` added 27-rank-ladder-rework.md Phase 1 — milestones granted NO xp
      // before (this upsert simply omitted the column), which is why the rank
      // ladder had no burst source and measured attendance alone. Not
      // retroactive: reward_events is append-only and this upsert is idempotent
      // on `milestone:<day>`, so a milestone already claimed keeps its 0 xp.
      { source: 'milestone', ref: `milestone:${day}`, coins: coinsToGrant, xp: reward.xp ?? 0, freezes: freezesToGrant },
      { onConflict: 'user_id,source,ref', ignoreDuplicates: true }
    )
    .select();
...
// One-time new-user welcome bundle (28-onboarding-welcome-bundle.md) — coins +
// freezes + the Glitch card theme, granted once at onboarding completion
// (lib/onboarding.js's finish()). Idempotent on (user_id,'welcome_bundle',
// 'welcome_bundle'): a repeat call (a re-run of finish, a retry) no-ops rather
// than re-granting. Same shape as claimTrophy — the freeze portion routes
// through clampFreezeGrant like every other freeze grant (a brand-new account
// holds 0 and is nowhere near the cap, but this keeps the single grant path).
// See WELCOME_BUNDLE in lib/rewards.js for why this bounded coins-at-signup
// exception exists. The theme grant is a SEPARATE row keyed by the theme id,
// exactly as claimMilestone/claimSpin/claimTrophy/claimRank all do it, so
// useCardThemes' ownedIds reads it identically.
export async function claimWelcomeBundle() {
  const { freezes: freezesToGrant, bonusCoins } = await clampFreezeGrant(WELCOME_BUNDLE.freezes);
  const coinsToGrant = WELCOME_BUNDLE.coins + bonusCoins;

  const { data, error } = await supabase
    .from('reward_events')
    .upsert(
      { source: 'welcome_bundle', ref: 'welcome_bundle', coins: coinsToGrant, freezes: freezesToGrant },
      { onConflict: 'user_id,source,ref', ignoreDuplicates: true }
    )
    .select();

  const themeId = WELCOME_BUNDLE.themeId;
  if (themeId) {
    await supabase
      .from('reward_events')
      .upsert({ source: 'theme_grant', ref: themeId, coins: 0 }, { onConflict: 'user_id,source,ref', ignoreDuplicates: true });
...
// Grants a rank's card theme on arrival (27-rank-ladder-rework.md Phase 2).
// Same idempotent-upsert shape as claimTrophy, with ONE deliberate difference:
// the `rank` row carries coins: 0, xp: 0 — it is purely an idempotency marker
// and a ledger record that the rank was processed, NOT a payout.
//
// Ranks must never pay currency, and this is the line most likely to be
// "improved" later by someone seeing a zero and assuming it's a stub. It is
// not. Rank is derived entirely from XP, which is derived entirely from
// actions that were already paid for (a logged day, a milestone, a trophy) —
// paying again at the rank boundary is a third payment for one behaviour, the
// same double-pay TROPHY_REWARDS already refuses for Streak Keeper. Coins are
// also deliberately scarce against shop sinks (see lib/rewards.js invariant 4)
// and are now real-money purchasable, so a new faucet here undercuts the
// store. The reward for a rank is the THEME, and the freeze cap that came with
// getting here. See 27-rank-ladder-rework.md's Context.
//
// A rank with no RANK_THEME_GRANTS entry (six of the nine) is a no-op —
// nothing to grant, nothing recorded, no marker row.
export async function claimRank(rankId) {
  const themeId = RANK_THEME_GRANTS[rankId];
  if (!themeId) return { error: null, isNewClaim: false, themeId: null };

  const { data, error } = await supabase
    .from('reward_events')
    .upsert(
      { source: 'rank', ref: rankId, coins: 0, xp: 0, freezes: 0 },
      { onConflict: 'user_id,source,ref', ignoreDuplicates: true }
    )
    .select();

  // The theme itself — a separate row keyed by the THEME's id, so
  // useCardThemes' ownedIds query reads it identically to a theme_buy or any
  // other theme_grant. Attempted unconditionally; a replay just no-ops on the
