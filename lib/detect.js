...
// (the module simply isn't linked there). Gate on all three: the IS_EXPO_GO
// check already used in lib/notifications.js, Platform.OS (didn't matter
// while this app was Android-only in practice, but matters now that an iOS
// release is a stated goal), and AUTO_DETECT_ENABLED — the lite APK variant
// (app.config.js's FLO_VARIANT=lite) excludes this module from autolinking
// entirely, so requiring it there would throw at app boot for every user,
// not just fail to render a feature.
const AUTO_DETECT_ENABLED = Constants.expoConfig?.extra?.autoDetectEnabled !== false;
...
// Distinct from isSupported(): that answers "can detection run on this
// device/environment" (platform + Expo Go + variant); this answers "does
// this build even ship the feature" (variant only) — Settings uses this one
// to decide whether to show the Transaction Detection section at all, vs.
// showing it with an explanatory fallback (isSupported() false, this true).
export function isAutoDetectVariant() {
...
// Human-readable labels for DEFAULT_ALLOWED_PACKAGES. Lives here, next to the
// list it describes, because it is the *consent disclosure* — the text that
// tells the user which apps FLO reads — and it is now rendered in two places
// (app/settings.js's Transaction Detection card and the onboarding detect
// step, 07-onboarding.md Phase 3). Two copies could drift, and a stale copy
// here doesn't just look untidy: it makes the disclosure a lie.
//
// Must stay in sync with DEFAULT_ALLOWED_PACKAGES above. Messages is the
// personal-use-only entry (see PERSONAL_USE_EXTRA_PACKAGES) — remove this
// line too if that one goes before a store build.
export const WATCHED_APP_LABELS = {
