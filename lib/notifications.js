...
export function useNotificationSync() {
  ...
  //
  // The EVENING nudge is the one exception (18-gamification-ritual-and-
  // ledger.md Phase 3, data.trigger — added server-side in send-push's
  // sendNudges): it already only fires when the user hasn't logged today, and
  // now doubles as the entry point to the close-the-day ritual (TodayCard,
  // which itself only renders past this same evening_reminder_time
  // threshold) — landing on Home puts the user right where that card is,
  // rather than skipping past it into the add-transaction sheet.
  const handleResponse = useCallback(
    (response) => {
      const data = response?.notification?.request?.content?.data;
      if (!data) return;
      if (data.type === 'nudge' && data.trigger === 'evening') {
        router.push('/');
        return;
      }
      if (data.type === 'nudge') {
