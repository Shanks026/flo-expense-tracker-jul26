...
export async function rescheduleAll({ bills }) {
  ...
  // Copy is still the original generic string this phase — visibility only.
  // 05-koban-engagement.md Phase 3 replaces this block with the streak-aware
  // rolling window (a 30-day loop of dated notifications split across
  // CHANNELS.nudge/CHANNELS.recap). Every slot lands on `nudge` until then.
...
export async function getScheduledSummary() {
  ...
  const items = scheduled.map((req) => {
    ...
    if (trigger.type === 'daily' || (trigger.hour != null && trigger.minute != null)) {
    ...
    return { title: req.content?.title ?? '(no title)', when, channelId: trigger.channelId ?? '—' };
  });
...
export function useNotificationSync() {
  ...
  // No settings state held here on purpose — rescheduleAll reads them from
  // AsyncStorage itself. Holding a copy here is exactly what caused the
  // stale-settings schedule-wipe bug documented on rescheduleAll above.
  useEffect(() => {
    if (loading) return;
    rescheduleAll({ bills });
  }, [bills, loading]);
