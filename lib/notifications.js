...
import { buildReminderPlan, toneFromCommitment } from './koban';
import { getReportSettings, toExpoWeekday } from './reports';
import { supabase } from './supabase';

// 12-personal-onboarding.md Phase 3 — the commitment answer's one real
// effect: Koban's nudge tone. Mirrors fetchStreak's own null-userId guard
// (hooks/useStreak.js) — 'default' tone is exactly the pre-existing pools.
async function fetchCommitmentTone(userId) {
  if (!userId) return 'default';
  const { data } = await supabase.from('profiles').select('onboarding_answers').eq('id', userId).maybeSingle();
  return toneFromCommitment(data?.onboarding_answers?.commitment);
}
...
const CHANNELS = {
  ...
  reports: 'flo.reports.ready', // HIGH — heads-up, sound, vibrate (11-reports.md Phase 2)
};
...
async function doRescheduleAll({ bills, userId }) {
  ...
  if (settings.dailyReminder.enabled) {
    ...
    const tone = await fetchCommitmentTone(userId);
  ...
  // 11-reports.md Phase 2. Both WEEKLY and MONTHLY are genuine repeating OS
  // triggers (confirmed against the installed expo-notifications version's
  // type defs, not assumed) — set once, fire forever, no rolling reschedule
  // needed the way bills/the daily reminder require. The tap routes via the
  // same data.route + useNotificationSync mechanism as every other
  // notification here; no queue-drain needed (that's the native
  // auto-detect module's problem, not this one's — this is JS-scheduled).
  //
  // Known limitation, not a bug: MonthlyTriggerInput's `day` has no clamping
  // for short months (unlike reportDueMoment() in lib/reports.js, which
  // deliberately clamps 31→28/29/30). A day-31 cadence simply won't fire in
  // Feb/Apr/Jun/Sep/Nov. The Home card and bell alert are unaffected — both
  // are computed independently via reportDueMoment and stay correct — so this
  // only ever under-delivers the best-effort push, never the reliable channel.
  const reportSettings = await getReportSettings();
  if (reportSettings.cadence === 'weekly') {
    await Notifications.scheduleNotificationAsync({
      content: {
      ...
      trigger: {
        ...
        weekday: toExpoWeekday(reportSettings.weekday),
        hour: reportSettings.hour,
        minute: reportSettings.minute,
        channelId: CHANNELS.reports,
      },
    });
  } else if (reportSettings.cadence === 'monthly') {
    await Notifications.scheduleNotificationAsync({
      content: {
        title: 'Your monthly report is ready',
        body: 'Tap to see how your month went.',
        data: { route: '/report' },
        priority: Notifications.AndroidNotificationPriority.HIGH,
        sound: true,
        color: '#BBDC12',
